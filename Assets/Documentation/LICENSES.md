# Asset Licenses

## Game-icons.net SVG Assets

License: Creative Commons Attribution 3.0 Unported (CC BY 3.0)  
License URL: https://creativecommons.org/licenses/by/3.0/  
Source: https://game-icons.net

The following assets are used in this project:

- Spaceship by Delapouite
- Alien Bug by Delapouite
- UFO by Delapouite
- Asteroid by Delapouite
- Two Coins by Delapouite
- Laser Gun by Delapouite
- Explosion Rays by Delapouite
- Open Treasure Chest by Delapouite
- Alien Skull by Lorc
- Crystal Growth by Lorc
- Checked Shield by Lorc

Attribution format requested by Game-icons.net:

Icons made by Delapouite and Lorc. Available on https://game-icons.net under CC BY 3.0.

## 8x8 Space Shooter Asset Pack

Source: https://gvituri.itch.io/space-shooter
Author: Gustavo Vituri

The source page states that the sprites may be used in personal and commercial projects
and may be modified. Credit is not required, but is included in `CREDITS.md`.
The original pack must not be redistributed as a standalone asset pack; these files are
only included as integrated game artwork.

## Press Start 2P Font

Source: https://fonts.google.com/specimen/Press+Start+2P
Designer: CodeMan38

`PressStart2P-Regular.ttf` is distributed under the SIL Open Font License, Version 1.1.
The font is bundled for in-game UI and damage-number text only.

## Kenney Planets

Source: https://kenney.nl/assets/planets
Author: Kenney

The original planet artwork is licensed under CC0 1.0 Universal. The eight included
planet images are downsampled derivatives used as integrated game artwork.

## Asset Search Notes

Itch.io's free pixel-art/UI listings were reviewed alongside Kenney and OpenGameArt.
Gustavo Vituri's 8x8 Space Shooter Asset Pack was selected because it supplies a cohesive
set of pixel ships, enemies, projectiles, backgrounds, effects, and UI elements under a
commercial-use-friendly permission statement. Kenney's CC0 Planets pack was added for
varied planet visuals, then downsampled to preserve the game's pixel presentation.
